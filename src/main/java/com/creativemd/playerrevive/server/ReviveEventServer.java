...
public class ReviveEventServer {
	...
	public void playerDied(LivingDeathEvent event)
	{
		if(event.getEntityLiving() instanceof EntityPlayer && isReviveActive() && !event.getEntityLiving().worldObj.isRemote && event.getSource() != DamageBledToDeath.bledToDeath)
		{
			...
			player.getServer().getPlayerList().sendChatMsg(new TextComponentString(String.format(I18n.translateToLocal("playerrevive.chat.bleeding"), player.getDisplayNameString())));
		}
	}
